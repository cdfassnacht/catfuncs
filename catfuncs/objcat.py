...
# ## The functionality in select_good_ast needs to be ported somewhere else
...
class ObjCat(Table):
    ...
    def load_from_file(self, incat, catformat='ldac', verbose=True,
                       namecol=None, racol=None, deccol=None, rafield=None,
                       decfield=None, usecols=False):
    ...
    def get_radec(self):
        ...
        if self.radec is None:
            ...
            print('         not found.  Please check the format of your '
                  'catalog.')
            print('')
    ...
    def read_centpos(self, posfile, verbose=False):
        ...
        if verbose:
            ...
            print('Coordinates: %s %s' %
                  (radec.ra.to_string(unit=u.hourangle, decimal=False, sep=':',
    ...
    def print_ccmap(self, outfile, verbose=True):
        ...
        for i in range(self.nmatch):
            f.write('%8.2f %8.2f  %11.7f %+11.7f\n' % (self.matchx[i],
                    self.matchy[i], self.matchra[i], self.matchdec[i]))
        f.close()
    ...
    def match_xy(self, xa, ya, max_offset=None, xcol=8, ycol=9, verbose=True):
        ...
        for i in range(2):
            ...
            dxmed = np.median(self.matchdx)[0]
            dymed = np.median(self.matchdy)[0]
            ...
            dxmed = np.median(dxm)[0]
            dymed = np.median(dym)[0]
            if verbose:
    ...
    def match_fits_to_ast(self, fitsfile, astcat, outfile=None, max_offset=None, 
        ...
        """
        raa, deca, xa, ya, astmask = \
            select_good_ast(astcat, hdr, racol, deccol, edgedist)
        if verbose:
